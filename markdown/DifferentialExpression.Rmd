...
Let us start with an example identifying the genes that are differentially expressedbetween two diets **Chow** and **High fat**.
...
par(mfrow=c(1,1))
plotMA(res_3reps, main="M-A Plot: 3 Samples per group", ylim=c(-2,2))
```
```{r top_gene_plots, results="hide"}
par(mfrow=c(2,3),las=1)
top5_genes= rownames(resOrdered_3reps[1:6,])
...
```{r sample_size_6, results="hide"}
sampleSize = 6
```
Later on we will see the effect of sample size by varying it.
```{r subset_exp_6, results="hide"}
diet_DE = c(male_chow[1:sampleSize],male_hf[1:sampleSize])
exp_design_diet_DE= exp_design[diet_DE,]
exp_design_diet_DE
exp_diet_DE=exp.all[,diet_DE]
...
```
Let us create data frames for DESeq2 object
```{r dataframe_deseq2_6, results="hide"}
### colData contains the condition/group information for Differenetial expression analysis
colData <- DataFrame(group = factor(exp_design_diet_DE$diet))
```
```{r deseq2_obj_6, results="hide"}
### Create DESeq2 object using expression and colData
dds_6reps <- DESeqDataSetFromMatrix(countData = as.data.frame(round(exp.dietDE)),
         colData = colData, design = ~ group)
dds_6reps <- DESeq(dds_6reps)
res_6reps = results(dds_6reps)
resOrdered_6reps <- res_6reps[order(res_6reps$padj),]
head(resOrdered_6reps)
...
```
```{r MA_plot_6}
plotMA(res_6reps, main="M-A Plot: 6 Samples per group", ylim=c(-2,2))
```
```{r top_gene_plots_6}
top5_genes= rownames(resOrdered_6reps[1:6,])
par(mfrow=c(2,3),las=1)
for (i in 1:length(top5_genes)){
  g_id = top5_genes[i]
  data <- plotCounts(dds_6reps, gene=g_id, intgroup=c("group"), returnData=TRUE)
  p <- ggplot(data, aes(x=group, y=count, color=group))
  p <- p+ ggtitle(g_id)
  p <- p+ geom_point(position=position_jitter(width=.1,height=0), size=3)
...
```{r pval_hist_6}
hist(res_6reps$pvalue,breaks=100,col="grey", xlab="p-value",main="p-value Histogram: 6 Samples per group")
```
